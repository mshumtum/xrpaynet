...
class CreateForgotPassCubit extends Cubit<BaseState> {
  CreateForgotPassCubit() : super(const BaseInit());

  clearState() {
    emit(const BaseInit());
  }

  createNewPassword({required String password, required String token}) async {
    emit(BaseLoading(
        state.main.copyWith(status: FormSubmissionStatus.inProgress)));
    try {
      CreateForgotPasswordRequest request =
          CreateForgotPasswordRequest(password: password, token: token);
      ApiService("");

      dynamic response = await ApiService.hit(
          url: Uri.parse(forgotPassword), body: request, header: {});
